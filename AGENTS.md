...
├── pipeline.py     # Shared local-entry pipeline helpers used by CLI and web routes
...
13. Codex slash commands: `/summarize` (single-paper, reuses existing `prompt.py` templates), `/import-summary`, `/lit-review`, `/compare` (multi-paper, uses new synthesis prompts). (See R4, R5)
