...
2e. ~~[RELIABILITY] Make edge-tts the default narration backend.~~ (2026-06-13) Complete-paper MLX generation remained vulnerable to isolated truncated chunks even after adding smaller sentence-aware chunks and semantic quality validation. edge-tts is now the reliable default; MLX remains an explicit local opt-in with validation and edge fallback.
