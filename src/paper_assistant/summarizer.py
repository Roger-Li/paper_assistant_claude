...
from paper_assistant.prompt import SYSTEM_PROMPT, USER_PROMPT_TEMPLATE
...
def format_summary_file(metadata: PaperMetadata, summary: SummarizationResult) -> str:
    ...
    header = (
        ...
        f"published: {metadata.published.isoformat()}\n"
        f"url: {metadata.arxiv_url}\n"
        f"---\n\n"
        f"# {metadata.title}\n\n"
    ...
    return header + summary.full_markdown
